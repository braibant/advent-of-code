...
fn distance(center : &Point, asteroid :& Point) -> f64{
    norm2(&Point {x : center.x - asteroid.x, y : center.y - asteroid.y} )
}

fn part2(asteroids : &Vec<Point>, center : &Point, nth: usize) -> Point {
    let mut asteroids : Vec<_> = 
    asteroids.iter().map(|asteroid| (asteroid.clone(),  angle(&center, &asteroid), distance(&center, &asteroid))).collect();
    ...
    loop {
        ...
        while asteroids[0].1 == theta {
            ...
            if da < db {asteroids.push((b, theta, db))} else {
